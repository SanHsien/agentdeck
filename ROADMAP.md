...
這份路線圖描述建議的產品方向、階段順序與完成條件。階段名稱代表依賴順序，不綁定特定版號或日期；當前缺陷仍以 [`REVIEW_Claude.md`](REVIEW_Claude.md) 為準，已完成的歷史則看 [`CHANGELOG.md`](CHANGELOG.md)。
...
目前：功能完整但仍有發版與 UI 邊角
  │
  ├─ 已完成基線：先消除已知不可信狀態
  ├─ Phase A：每個資料來源都能說明健康狀態與修復方式
  ├─ Phase B：狀態與診斷可供 PowerShell／其他本機工具使用
  ├─ Phase C：AI 圓桌能產生可搜尋、可續用的工作產物
  └─ Phase D：新資料來源與發版流程有明確契約
       │
       ▼
...
只有歷史索引與重開流程實際被使用後，才決定是否把這兩項排進後續 MINOR，避免 Phase C 同時承擔太多新互動。
...
效能數字先在 Phase A 建立基準，再把可重現的數字寫成 gate；不要先憑感覺訂一個漂亮但沒有測量方法的毫秒數。
...
1. **完成 Phase A 的修復閉環**：補齊首次啟動、未登入、舊 hook 與服務失敗的原因／下一步，並完成 Codex 事件來源決策與效能基準。
2. **啟動 Phase B 的穩定 JSON 介面**：先交付經過 redaction 與 schema 測試的 `--status --json`，讓 PowerShell 不必解析人類文字。
3. **啟動 Phase C 的討論復原能力**：先保證失敗／取消仍完整存檔，再建立歷史索引、搜尋與重開檢視。
