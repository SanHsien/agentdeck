...
- **4 款視覺主題：** Classic、Catppuccin（含 Latte／Frappé／Macchiato／Mocha 四款配色）、彩繪玻璃與摺紙。所有主題共用同一份行為核心，外觀之外的邏輯只有一份。
...
系統匣圖示會隨 Claude 額度百分比更新；提示文字摘要 Claude 與 Codex 的各視窗。左鍵用 WebView2 開啟 4 款主題面板；右鍵可切換面板、重新整理、設定開機自啟、檢查更新與結束。
