...
### 修正
- **狀態列開關關著時解除安裝，現在會還原安裝前的狀態列。** 開關原本把目前的 statusLine 暫存在舊名 `usage.previousStatusLine`，解除安裝時沒取回，安裝前的設定也就沒還原。現在暫存到 `agentdeck.disabledStatusLine`（仍讀得到舊位置），解除安裝也會一併還原 Antigravity 狀態列。取自上游 `24f9ad8`。
- **離線價格表補上 Opus 5.5、gpt-6-sol 與 gpt-6-luna。** 價格快取過期時這些模型的用量原本一律算成 $0；報表表格也補上「Opus 5.5」短名。取自上游 `752d7f1`、`07439a4`。
- **Grok 花費在總紀錄檔不存在時仍會補回。** 原本 `unified.jsonl` 不存在就回傳空清單，各 session `updates.jsonl` 的花費全部遺失；紀錄沒變動時也改為沿用上次結果。取自上游 `e1844a2`、`c9f3df7`。
- **精簡模式與接續對話 hook 的備用文字與介面語系同步。** sidecar 遺失時露出的內建文字原本落後 `i18n.json`；已安裝的 hook 會在下次啟動時自動換新。取自上游 `81de184`、`2b00fdb`。
- **session 檔很多時，產生報表不再每次整份重讀。** 報表解析的檔案快取上限從 512 提高到 4096。取自上游 `c88b0ad`。

### 文件
- **README 補上 Windows 首次執行被 SmartScreen 擋下時的放行步驟。** 取自上游 PR #145。

### 變更
- **依賴宣告下限對齊實測版本。** 依賴新鮮度檢查審核 `ruff`（`>=0.16.5` → `>=0.16.8`）與 `pyinstaller`（`>=6.22.2` → `>=6.22.3`），lock 與宣告同步升級，新版對現有 208 個檔案零新增發現，閘門與測試全數通過。

## [0.42.1] - 2026-09-19

### 修正
- **換帳號後不再沿用前一個帳號的額度快取。** Claude Code 會在 `~/.claude.json` 的 `cachedUsageUtilization` 記下抓取時的 `accountUuid`。原本讀這份快取時沒有比對 `oauthAccount.accountUuid`，在 Windows 上以快取為主要來源時，切換帳號會誤讀前一個帳號的百分比。取自上游 `f435f13`。
- **Claude 設定檔支援 UTF-8 with BOM。** Windows 編輯器可能在儲存 `settings.json` 時加入 UTF-8 BOM（`\xef\xbb\xbf`），造成 `json.load()` 解析失敗。`setup_hook.py` 改採 `utf-8-sig` 讀取相容。取自上游 PR #139（`96ed3f7`）。
- **週額度警示改採雙速度門檻與餘裕要求，消除密集衝刺誤報。** 週額度原本只拿短窗（30～60 分鐘）速度外推 7 天，短時間衝刺即容易誤報。引入 `assess_weekly_quota()`，要求短窗與整週平均速度皆預測重置前耗盡（或短窗 1 小時內用完之緊急分支），且整週平均預測須早於重置時間的 80%（保留 20% 餘裕）才警示，文案同步調整。取自上游 PR #138（`d964f8d`、`d99dbbc`）。
- **啟動與日常操作不再閃黑窗。** 這個程式沒有自己的主控台，所以每次呼叫 `git`、`claude`、`clip` 或 Antigravity CLI 都會在 Windows 上彈一個黑窗——專案名稱解析、額度輪詢各算一次，累積成整段時間的干擾。17 個呼叫點裡原本只有 1 個擋住，現在全部統一由 `creation_flags()` 處理，並用 AST 掃描測試釘住新的呼叫點不會漏。取自上游 `2a1996e`。
- **狀態列 forwarder 的修正現在也會進到已安裝的副本。** 自我修復原本只換新狀態列那一份，forwarder 只有重裝才會更新。
- **系統匣提示過長不再讓面板停止更新。** Windows 的 `szTip` 上限是 128 個字元，pystray 超過就丟例外，而那個例外落在更新路徑裡——四列供應商加上本地化的視窗標題就會超過。提示現在截到 127 字元並補省略號。取自上游 `8eacc3b`。
- **狀態列的 token 數字不再出現不存在的單位。** `999,500`～`999,999` 原本會顯示 `1000k`；門檻改用實際進位點（`999,500` 進 `M`、`999,950,000` 進 `B`），並補上 `B` 這一級。`ui/tables.py` 先前已修過同類問題，這次補的是兩個狀態列腳本。取自上游 `45b27ee`。
- **精簡模式不再壓過你明確要求的詳細解說。** 每則訊息注入的提醒句補上「使用者明確要求詳細解說時以使用者為準」，開場指示也移除「允許不成句的斷句」那條——它會把解釋類回答壓成碎片。取自上游 `93b2f19`，雙語文字照本 fork 自己的措辭改寫。
- **狀態列修正現在真的會進到已安裝的副本。** `setup_hook` 是比對版號決定要不要換掉 `~/.claude` 底下那份，所以改了檔案卻沒升版號的修正等於沒發布。版號已同步升級，並加上測試釘住「常數與檔案版號必須一致」。同上游 `144ecac` 的教訓。
- **Grok 那一列的花費不再恆為 `$0.00`。** 改讀 Grok CLI 各 session 的 `updates.jsonl` 裡的 `costUsdTicks`，並把 `unified.jsonl` 滾動視窗外的舊請求補回成獨立紀錄。取自上游 `784eeb9`／`7a78e64`，以可逐字比對驗證的機械改寫移植。
- **離線價目表補上 Codex 與新 Claude 模型。** 網路價目表取不到時，`gpt-6-astra`、`gpt-5.6-terra`／`sol`／`luna`、`gpt-5.5`、`gpt-5.4-mini` 與 `claude-mythos-5-1` 的花費不再算成 0。取自上游 `47b0a64`。
- **token 顯示在四捨五入進位點不再出現 `1000.0K`／`1000.0M`。** 格式化門檻改用實際進位點，`999,950` 會顯示 `1.0M`，`999,950,000` 會顯示 `1.00B`；邊界案例已加入回歸測試。取自上游 `d665d67` 的格式化修正半部。
- **手動重建 Release 資產現在必須核對 tag 的完整 commit SHA。** `workflow_dispatch` 新增必填 SHA，checkout 也限定在 `refs/tags/<tag>`；若實際 checkout 的 commit 與輸入 SHA 不同，會在安裝依賴與建置前停止，不再讓移動或誤選的 tag 以 `--clobber` 覆蓋既有資產。

### 變更
- **系統匣的文字組裝搬到 `tray_text.py`。** `tray_icon_style` 與 `build_tooltip` 是純函式，移出 `wintray.py` 後該檔降到 1816 行，受控上限也一併調降。對使用者沒有行為差異。

### 文件
- **介紹頁的面板數量改為正確的 4 款。** 英文與繁中 metadata、功能卡和截圖區原本仍寫 10 款；現在與實際提供的 Classic、Catppuccin、彩繪玻璃、摺紙一致。
...
### 修正
- **精簡模式的每則提醒 hook 裝好之後就再也不會更新。** `_self_heal_terse_reminder()` 只檢查 script 或 UserPromptSubmit 設定「在不在」，兩者都在就直接返回——已安裝但過期的那份永遠不會被換掉，使用者升級 agentdeck 也拿不到新版提醒詞。同一支 `session_hooks.py` 裡另外三個 hook（SessionStart terse、Codex terse、resume）本來就會比對版本，`usage_terse_reminder.py` 也早有 `__version__`，只是沒人拿來比。新增 `TERSE_REMINDER_HOOK_VERSION` 與 `_installed_terse_reminder_version()`，缺件的還原路徑不變，多一條「在但過期就重裝」並記 `update_terse_reminder_hook`。取自上游 `1ec6fe4`。
- **精簡模式指令自相矛盾，同一段話既要求開場白帶 🐾 又禁止表情符號。** 另一處把「旁白工具呼叫」一竿子禁掉，跟「開工具前先說一句要做什麼」的常見用法打架。兩句都改寫：表情符號限縮成「除了開頭那句招呼，內文不放」，工具部分改成不要複述工具名稱或呼叫過程、但開工具前說明一句是可以的。**兩份副本一起改**——`usage_terse_mode.py` 的 `_DEFAULT_INSTRUCTION` 只是退路，真正送進模型的是 `i18n.json` 的 `terse_mode_instruction`（由 `_write_terse_sidecar()` 寫進 `~/.claude/agentdeck-terse-prompt.json`，`_load_instruction()` 優先讀它）。`TERSE_HOOK_VERSION` 推到 1.1，讓已經開著精簡模式的使用者在下次自我修復時拿到修好的文字。取自上游 `4222250` 的 prompt 半部；同一筆的語言偵測半部不適用，本 fork 的 `_detect_lang()` 早已回退 `_windows_system_lang()`，沒有預設落到英文的問題。
...
- **只使用 Claude Desktop 時，Claude Code 用量與重置資訊會跟著本機歷史更新。** Desktop 以 stream-json 執行 Claude Code，不會刷新終端專用的 statusLine hook；agentdeck 因而曾一直重播舊的 `agentdeck-status.json`。現在會讀 Desktop 持續寫入的本機 `plan-usage-history.json`，顯示資料來源與更新時間，並從同一組織的用量下降事件推估 5 小時／7 天視窗；推估值以 `約` 明確標示，沒有足夠證據時仍顯示 `--`。忙碌中的手動重整會合併成一次尾隨刷新，不再直接丟棄；全程不呼叫 Anthropic usage API。
- **同步上游的正確性與安全修正。** Hook 擁有權改用完整腳本檔名判斷，不再把 `agentdeck-statusline-pro.py` 或備份檔誤認為可覆寫的 agentdeck hook；HTML 報告的專案圓餅會把未列出的 token 計入「其他」，月份標籤不再因間距碰撞漏月，未完成週也不再拿來做週增減比較；Antigravity statusline 不再重複顯示模型名稱中的 reasoning effort。
...
### 變更
- **`ruff` 下限 0.16.4 → 0.16.5**，lock 一起升。依賴新鮮度檢查的唯一一筆 REVIEW UPDATE；新版對現有 200 個檔案零新增發現，`dev_check.ps1` 全綠（1537 passed）。
- **精簡模式現在也要求白話，不只要求短。** 原本的指令只說「短」，回覆就靠塞術語變短——短而難懂。新增三條可檢查的要求：挑最口語的說法（「先存檔」不要「先持久化」）、非用不可的技術詞第一次出現補十個字以內的白話解釋、收尾就講做了什麼／成功沒／下一步。每則都送的提醒 hook 只多「用白話」三個字，逐則節省不受影響。取自上游 `ec89500`。
...
### 修正
- **停手後速率不會自然衰減，一路卡在高負載**:burn rate 的分母原本是「最後一筆減第一筆記錄」的跨度，不含最後一筆之後的閒置時間。本機重現:密集操作 10 分鐘（56,100 tokens）後停手 40 分鐘,舊算法仍讀 5,610 tokens/min（**Active**）,真實速率只有 1,122（Normal）——而且會一路卡著,直到記錄滑出 1 小時窗才恢復。改成從「現在」起算。
  - 既有測試原本沒固定「現在」,隱含依賴舊分母;已補上凍結時間點維持它們原本要測的語意。另加兩條測試,把**同一份資料**在「進行中」與「停手 40 分鐘後」的兩種答案一起釘住——只擋一邊的話,修正會退化成「永遠不報高負載」。
- **摺紙面板的重置時間落在右下摺角上幾乎看不見**:`.reset` 的字色與摺角最深處實算 **WCAG 對比 1.63**（AA 小字要 4.5）。改用深色字加三層淺色光暈,實算 **12.11**,疊在什麼底色上都讀得到。

### 變更
- **補上面板定義的一致性閘門**:面板 id、i18n key、HTML 檔名與初始高度都在單一來源 `panels/registry.py`,但它驗不了自己的一件事——**檔名是否還指向存在的檔案**。新增三道測試:高度涵蓋擴到人才市場、登記的 HTML 檔必須存在、不得有指向已移除面板的孤兒高度（v0.40.0 移除九張主題正是這種風險）。
...
### 新增
- **Antigravity CLI 也有狀態列了**:面板上原本的「終端」開關現在同時控制 Claude Code 與 Antigravity 兩邊,不另外加按鈕。狀態列顯示 5 小時與本週配額、對話窗用量與模型,**完全不連網**——資料是 agy 自己從 stdin 餵進來的。
  - 動手前先驗證平台支援:`agy.exe` 內含 `"statusLine"` 等字串,設定檔在 Windows 上是同一個路徑。**上游把 `/usr/bin/python3` 寫死**,在 Windows 上會裝出一條永遠跑不起來的指令,改用與 Claude hook 相同的直譯器解析。
  - 移除時**刻意不刪腳本檔**:Antigravity 只在啟動時讀一次設定,刪檔會跟正在啟動的 CLI 搶時間,中獎的視窗整個生命週期都掛著錯誤。

### 變更
- **Codex 狀態列加上 git 分支與已用 token**:段位從 5 個增為 7 個。改設定前先讀安裝的 codex-cli 執行檔,確認七個段位識別字全部存在——不被支援的段位會安靜地少一塊,不會報錯。
  - 一併移植**升級安全機制**:原本的安裝流程會把「我們自己上一版寫進去的設定」當成使用者的原始設定備份起來,之後移除時就還原出一個他從沒選過的狀態列,而且蓋掉他真正設定的唯一副本。現在程式認得自己過去的輸出,就地升級、不動既有備份。已安裝的使用者會在下次啟動時自動升級。

### 文件
- 新增 [`docs/SIGNING.zh-TW.md`](docs/SIGNING.zh-TW.md):Windows 執行檔的 SignPath 程式碼簽章申請與接線步驟。步驟 1～3 需維護者本人向 SignPath Foundation 申請（人工審核、要具名自然人）,4～5 是 repo 內的改動。

## [0.40.1] - 2026-08-13

### 修正
- **一則大訊息會讓「還能撐多久」的預估直接崩掉**:預估原本逐間隔算速率再用 EMA 合併,最新那一段永遠佔一半權重、且不隨間隔長短加權。Claude 的百分比是階梯式跳動,一則大訊息落在短輪詢間隔就產生極大的瞬時速率。**實測**:穩定 0.5%/分鐘燒十分鐘後,一則訊息在 5 秒內加 7%,預估從 32 分鐘掉到 **0.9 分鐘**——面板會對你大喊「不到一分鐘就用完」。改用窗口平均斜率。修完同一組資料是 28 分鐘,而真正在加速的燃燒仍然會提早警告(實測 10 分鐘)。（移植自上游 `4eb0e5e`）

### 變更
- **執行檔補上版本資訊**:先前 `agentdeck.exe` 完全沒有版本資源,Windows 的「檔案內容」分頁一片空白,拿到一個下載來的 exe 無法在不執行它的情況下辨識版本。版本號直接取自 `pyproject.toml`,不另開第二個來源。（概念來自上游 `07812bb`;SignPath 簽章本身未採用——需要維護者自行申請 OSS 方案,且上游自陳該流程尚未在 CI 實跑過。）

## [0.40.0] - 2026-08-12

### 變更
- **面板改用共用行為核心,主題只剩外觀**:九張舊主題共 11,263 行,其中 3,675 行是 `<script>`,彼此相似度 **89–96%**——每張主題真正獨特的只有 CSS,另外約 350 行邏輯是抄的。這個成本一再現形:移除最小化按鈕、拖曳區、`data-i18n` 契約,每一項都要改九次;而 World Cup 面板完全沒有 Antigravity 欄位,一路出貨到有人 grep 才發現。現在行為集中在 `panel_core.js`,面板只帶自己的 CSS 與少量 hook。
- **主題改為 4 款**:Classic(預設)、**Catppuccin**(含 Latte／Frappé／Macchiato／Mocha 四款配色,系統匣可切換)、**彩繪玻璃**、**摺紙**。舊的九張移除。移植前逐一確認三張新主題都完整支援 Antigravity(19／13／17 處),不會重演 World Cup 那種少一整個來源的情況。
  - 原本用舊主題的人會自動回到預設,不需要任何操作。

### 修正
- **面板塞不下時改為捲動,不再裁掉內容**:新面板把 `html, body` 釘在 `100vh; overflow: hidden`,在比面板矮的螢幕上每張卡會被壓縮並裁掉自己的最後一列——而 `overflow: hidden` 表示被裁掉的部分**沒有任何辦法看到**。實測對照:舊面板不裁,四張新面板全部裁。現在超出的部分可以捲動。
- **視窗下緣不再壓到工作列**:高度上限原本先套用工作區、**之後才加上標題列高度**,於是外框剛好比工作區高出一整條標題列。改成先扣再取上限,並以外框高度夾邊界。
- **首次開啟改為錨定螢幕上緣**:面板是比工作區還高的長條,錨在下緣會把它推出螢幕,也讓要抓的標題列位置隨主題高度浮動。左右仍跟著工作列的位置走。實測(邏輯像素):視窗 `(1315, 12, 1695, 900)`、工作區 `(0, 0, 1707, 912)`。

## [0.39.4] - 2026-08-12

### 修正
- **一段肥的系統提示詞會讓燃燒速率誤判為 Heavy**：速率是用「總 token ÷ 時間跨度」算的,而跨度下限只有 1 分鐘,所以極短時間內的單一 `cache_creation` 會被除成看起來持續很兇的速率。**實測**:兩則訊息相隔 30 秒、其中一則 20,000 cache_creation,就被判成 Heavy,動畫照 Heavy 跑。

  下限改為對齊本專案自己的預測門檻（`burn_rate.MIN_FORECAST_SPAN_SECONDS`,5 分鐘）,而不是另寫一個數字——兩處對「多短算太短」若各有一個答案,遲早會漂開。同一組資料現在判為 Active;而連續 20 分鐘的真實重度燃燒**仍然是 Heavy**（只會壓低不會誤放的修正,跟拿掉功能沒兩樣）。（概念來自上游 `d92f683`）

### 維護
- 上游審視推進至 `49a0dfa`。**本 fork 與上游的面板架構已經分家**:上游把九張面板改用共用核心之後,連 `classic.html` 都不再定義 `window.usageApplyState`,而本 fork 九張面板都還定義它。因此上游此後的每一張新主題（Catppuccin、彩繪玻璃、摺紙）都不能直接複製——照抄會得到一張畫得出來、但永遠收不到額度資料也不回報高度的主題。三張合併為同一項後續工作,理由記在 `docs/UPSTREAM.md`。

## [0.39.3] - 2026-08-10

### 修正
- **「關於」對話框關不掉**：它以「無擁有者」的方式從背景執行緒開啟,所以 Windows 從不給它焦點,它會開在你正在看的視窗**後面**——按下「關於」看起來像什麼都沒發生。於是你再按一次,而**每按一次就多疊一個對話框**:實測連按三次就有三個,每一個都要各自關掉,這才是「關不掉」的真相。

  現在對話框由面板視窗擁有,並帶 `MB_SETFOREGROUND | MB_TOPMOST`——它會蓋在置頂的面板之上,而且面板在它開著時會變灰,一眼就看得出要先處理它。同時**同一時間只允許一個原生對話框**。實機驗證:連按三次現在只有一個;對話框的擁有者確實是面板、面板在它開著時被停用、關閉後恢復。

  另外「關於」不再擋住面板的 JS 橋接執行緒,面板在對話框開著時不會僵住。

### 維護
- 原生對話框的處理抽成 `win_modal.py`,讓 `wintray.py` 回到 1900 行上限之內(而不是抬高上限)。鎖是行程層級而非每個物件一把——只有一套 UI、一個使用者,第二個 modal 從來都不是正確答案。

## [0.39.2] - 2026-08-10

### 修正
- **設過 `CODEX_HOME` 的人看到的是一張空白 Codex 卡**：第二個帳號、容器化的 Codex——本程式有 11 處把路徑寫死成 `~/.codex`，那些設定下讀不到任何東西，而且**不會報錯**:「Codex 沒有用量」和「我們找錯地方」在畫面上長得一模一樣。全部改走新的 `codex_paths.codex_home()`。（移植自上游 `ab03f92`）路徑常數在啟動時決定，所以改了環境變數要重開程式才生效。

### 維護
- **發版流程新增版號守門**：打錯 tag（版號變舊、或重切一個已被取代的版本）原本會一路建置到底，唯一的症狀出現在使用者端——更新檢查比對版號後，安靜地判定「沒有新版可裝」。現在版號不是嚴格遞增就在建置前擋下。（概念來自上游 `45397d8`，但**沒有照抄**:上游的比較集合包含正在發的那個 tag，而本專案是先建立 release 再由 tag 觸發流程，照抄會擋掉每一次發版。）
- 上游審視推進至 v0.29.23。`codex_otel.trace_safe` 這條舊格式查詢路徑的說明已寫進 `CLAUDE.md`——並且是**自行量測驗證**的:本機 35,250 筆紀錄中它只佔 47 筆，全部集中在 2026-08-05 的 52 分鐘視窗內，而該表至今仍在寫入。共用面板核心與兩張新面板列為後續獨立工作，其餘不採用理由逐列記在 `docs/UPSTREAM.md`。

## [0.39.1] - 2026-08-10

### 修正
- **兩個行程同時編輯設定檔會掉紀錄，在 Windows 上還會直接出錯**：自我修復紀錄的寫入是「讀取 → 修改 → 存回」，而每一次存回本身都是原子的——**這正是問題看不見的原因**:檔案永遠不會壞，後寫的人只是把他讀到的舊副本寫回去，先寫的那筆就這樣消失。新增 `settings_lock.py`（POSIX 用 `fcntl`，Windows 用 `msvcrt`）把整段包起來，鎖檔獨立於狀態列那把，不必排在每分鐘數次的刷新後面。（移植自上游 `e16c5c0`）

  實測（四個真實行程各寫五筆）:**有鎖保留 20/20，無鎖只剩 5/20**。無鎖那組還會噴 `PermissionError: [WinError 5]`——Windows 上另一個行程開著目標檔時 `os.replace` 會失敗，所以同一個 bug 在本平台比上游描述的更嚴重。

## [0.39.0] - 2026-08-10

### 修正
- **AI 人才市場每次打開都說「元件尚未安裝」**：角色名冊只在「當前面板是人才市場」時才會被讀取，但切換面板後頁面是用**上一個面板建立的狀態**初始化的。於是內容永遠要等下一次背景輪詢（最多 60 秒）才出現——對使用者而言，這和功能壞掉沒有區別。現在開啟時會強制重建狀態。
- **卡片排序在新增額度來源時會被整組丟掉**：驗證原本要求存檔的排序長度剛好等於預設值，所以日後多一張額度卡，**每位既有使用者自己排的順序都會被判定無效、悄悄還原成預設**，而且沒有任何說明。改成只擋重複／未知 id，缺的卡片自動補到最後。（概念來自上游 `1ac8961`）
- **設定檔若是符號連結，寫入會把連結換成普通檔**：`os.replace` 換的是目錄項目，所以把 `~/.claude/settings.json` 放在 dotfiles repo 再連結過來的人，會發現它悄悄脫離了原本的 repo。現在先解析真實路徑再寫。（移植自上游 `e16c5c0`）
- **面板高度表停在舊值**：這張表是 WebView 回報真實高度**之前**的開場尺寸，值不對就會在每次開啟時看到一次跳動。數字**不是抄上游的**（那是 macOS 校正值，不同引擎畫的不同面板），而是實際量測本 fork 自己的面板：Windows 11、225% DPI、攔截 clamp 之前的原始回報值。

### 變更
- **AI 人才市場移出「面板」清單，改為系統匣選單的獨立項目**：它顯示的是角色卡與安裝按鈕，完全沒有額度資訊——列在主題清單裡讓那份清單謊稱自己提供的東西，也讓人可能因為「挑個外觀」而卡在裡面。它**刻意不會被記成開機面板**：開機開進人才市場，會蓋掉這個程式存在的理由。
- **移除 World Cup 2026 主題**：`world_cup.html` 完全沒有 Antigravity 的欄位，而其他九張額度面板各有 24–33 處——用那個主題的人會靜靜少掉一整個來源，畫面上沒有任何提示。上游同樣沒有、也未修正。它另外還把 `USAGE 2026` 寫死在 HTML 裡。視覺主題因此從 10 款變為 9 款。

### 維護
- **修掉 v0.38.0 留下的測試定時炸彈**：`tests/test_autoresume_wiring.py` 把基準時間寫死成 `2026-08-09 10:00 UTC`，而 `tick()` 讀的是真實時鐘。寫的當天全綠，隔一天就自己變紅、程式一行沒改。改為相對於真實時鐘取基準。
- 面板註冊表搬進 `panels/registry.py`，讓 `wintray.py` 回到 1900 行上限之內（而不是抬高上限）；`wintray` 明確 re-export，既有 import 位置不變。
- 上游審視推進至 v0.29.21；`e16c5c0` 只採用一部分，self-heal log 的跨行程鎖尚未做（Windows 沒有 `flock`，需要另抽共用鎖模組），理由與其餘六筆不採用逐列記在 `docs/UPSTREAM.md`。

## [0.38.0] - 2026-08-09

### 新增
- **額度重置後自動續跑（預設關閉）**：5 小時額度用完時工作就停在那裡，直到視窗換過去。開啟後，agentdeck 會在額度回來時自動把上次的工作接回去跑，跑完用系統通知回報。在偏好設定檔加 `"auto_resume": true` 啟用；可調 `auto_resume_trigger_pct`（預設 95，低於此值視為沒被卡住、不排程）、`auto_resume_weekly_ceiling_pct`（預設 85）、`auto_resume_lead_seconds`（預設 180）。
  - **為什麼用 Windows 排程而不是程式內的計時器**：機器休眠時程式內的計時器跟著凍結，而「整夜沒人在」正是這功能存在的理由。排程任務帶 `StartWhenAvailable`，錯過觸發時間會在喚醒後補跑。
  - **為什麼看絕對時間而不是等百分比掉下來**：狀態檔只在 Claude Code 刷新狀態列時才會被寫入，機器一閒下來數字就凍住。`resets_at` 是絕對時間戳，就算快照過期也仍然有效，所以排程是照它算的，不是靠偵測數字變化。
  - **週額度閘門**：續跑在 5 小時視窗重置時觸發，但花的是同一份 7 天額度。7 天用量達 `auto_resume_weekly_ceiling_pct` 時不排程，避免一個晚上把接下來幾天的額度用光。
  - 續跑用 `claude -p`（非互動）而非 `claude --resume`：排程任務沒有終端機，互動模式會卡住不動。上次工作的線索（最近的請求、commit、未完成待辦）由既有的 session resume 抽取邏輯生成，兩者不會對「工作停在哪」有不同說法。
  - 續跑指令要求跳過破壞性與不可逆操作、線索不足就停下說明，不自行擴大範圍。
...
### 修正
- **繁體中文介面有 52 個字串其實是簡體**：「暂无数据」「加载数据…」「消息」「会话」「时间」「项目」「总Token」「限额」——TUI、報告、面板、安裝訊息都有。本 fork 只出繁中與英文，繁中是預設語言，而這種錯誤**任何既有閘門都看不到**:鍵在、值非空、格式正確，一個寫錯字體的字串仍然是完全合法的字串。另修掉一則 CHANGELOG 內文與四處程式註解的簡體字。（移植自上游 `843505e`）
- **離線價目表的 Opus 價過期、而且完全沒有 Opus 5**：抓不到線上價目表時會退回內建表，但 `claude-opus-4-6`／`4-7` 還停在 $15/$75（實際已降到 $5/$25），`claude-haiku-4-5` 少算 20%，`claude-opus-5` **根本不在表裡**——離線時 Opus 5 的成本會安靜地算成 $0。數值不是照抄上游，是拿本機 `~/.agentdeck/pricing_cache.json`（2525 筆真實 LiteLLM 線上表）逐項核對，修完 8 個模型與線上表零差異。（概念來自上游 `c33e260`）
- 更新提醒的日期記錯:0.37.3 實際發布於 2026-08-06，而非 08-05。

### 維護
- **新增繁中字體閘門** `tests/test_i18n_traditional.py`。它是字元黑名單、不是轉換器——不可能知道「分布」該是「分佈」，但簡體專用字一出現就紅。繁簡同形字（目、置、配、段、消、加、字、周、布、余、退）刻意排除，另有一條測試守著這件事:會對正確文字誤報的檢查，最後會被關掉而不是被修好。
- **`i18n` 鍵比對新增兩條檢查**（概念移植自上游 `bc28b5a`，其 ja/ko 資料本 fork 不適用）:空字串翻譯會通過鍵比對卻上線成空白標籤；`{placeholder}` 與英文不一致會讓 `.format` 在執行期炸掉,而且只炸在使用該語言的人身上。
- 上游審視推進至 v0.29.18（`73b71d4`）；`86bde4a`、`57f207b` 與兩筆發版 chore 不採用，理由逐筆記在 `docs/UPSTREAM.md`。
...
### 修正
- **按「結束」後程式沒有真的結束**：`quit()` 是用 `destroy()` 關閉視窗的，而 `destroy()` 同樣會觸發 `closing` 事件——0.37.0 新增的「關閉即收進系統匣」處理常式把它一併取消了。結果是最糟的狀態:系統匣圖示已經停掉、視窗被隱藏，行程卻還活著，**沒有東西可以點、沒有辦法結束**，而單一實例鎖還告訴下一次啟動「已在執行中」。現在以 `stopping`（只由 `quit()` 設定）區分「使用者按了 X」與「程式要離開」。
- **從系統匣點圖示只跳出工作列按鈕，還要再點一次才顯示**：標題列的最小化會先把視窗設成 iconic，我們的處理常式接著隱藏它——於是視窗**同時是隱藏且最小化**。只呼叫 `show()` 恢復的是可見性，狀態仍是 iconic，所以只出現一顆工作列按鈕。現在 `show()` 之後接 `restore()`；順序不能反，`restore()` 對還在隱藏中的視窗不會有任何作用。

### 維護
- `on_closing` 移入 `panels/window_visibility.py`，與 `on_native_minimize` 放在一起——`wintray.py` 因此回到 1900 行上限之內，而不是把上限往上調。

## [0.37.1] - 2026-08-05

### 修正
- **訊息與對話框不再顯示舊程式名**：單一實例對話框寫著「usage 已在執行中；請點系統匣的 usage 圖示」，另外還有 9 組字串同樣殘留——狀態列未設定提示、報告頁尾與版本行、五則安裝／移除 statusLine 的訊息、TUI 標題、共存模式說明、Codex 資料延遲的說明。**英文側是重災區**：其中數則的中文早已改成 `agentdeck`，英文卻還留著舊名，等於同一句話兩種語言講的是兩個產品。狀態列提示同時修正了指令本身（`usage setup` → `agentdeck --setup`）。
- **報告頁尾與 self-heal 診斷訊息的程式名寫死在程式碼裡**：`ui/html_report.py` 與 `session_hooks.py` 直接把名稱寫進 f-string，不經 i18n，所以任何針對翻譯檔的檢查都看不到它們。

### 維護
- **新增品牌回歸閘門** `tests/test_i18n_branding.py`：去品牌化已經漏了三次，都是同一種漏法。檢查刻意收窄——產品名一律小寫，所以只有獨立的小寫 `usage` 算違規，標題大小寫的 `Usage` 是一般英文名詞；真的在講「用量」的 11 個鍵逐一列名，新增一個必須是刻意動作。另有一條測試防止白名單比它覆蓋的字串活得更久。實測會擋：注入一處舊名後閘門紅燈。

## [0.37.0] - 2026-08-04

### 變更
- **視窗改為有標題列，關閉與最小化都收進系統匣**：面板原本是無邊框視窗，只能靠自繪的 `−` 按鈕最小化，等於在系統標準之外另做一套視窗控制。現在改用一般標題列，最小化、關閉、Alt+F4 全部走同一條路——隱藏視窗、離開工作列與 Alt-Tab，系統匣圖示是唯一的回來路徑。**關閉不會結束程式**；離開程式的唯一入口是系統匣選單的「結束」，這正是單按一次 X 之所以安全的原因。
- **移除面板內的最小化按鈕**：標題列已經提供最小化，頁面內再放一顆就是同一件事的第二個控制項，而且就疊在真正的那顆下面。`minimize` 這個 JS 動作與 `minimize_window` 翻譯字串一併移除。
- **視窗停用最大化與縮放**：面板高度是依內容計算的，讓使用者拉大只會留下空白。`resizable=False` 同時移除最大化按鈕與可拖曳邊框。

### 修正
- **標題列不再吃掉面板底部**：`resize()` 設定的是**外框**尺寸，所以有邊框的視窗拿到內容高度後會正好短一個標題列，把最後幾列切掉。現在改為向即時視窗量測標題列與邊框的高度再加回去（在 225% DPI 下實測為 35 邏輯像素）；不寫死常數，因為它會隨 DPI 與使用者的標題列設定改變，而寫錯只會表現為「少了幾像素」這種無聲的錯誤。
...
### 變更
- **最小化改為收進系統匣,不再留在工作列**：面板的 `−` 按鈕原本呼叫 Windows 原生最小化,結果同一個視窗同時存在於系統匣與工作列——兩個入口、兩種行為,而且佔用它不需要的工作列空間。現在改為隱藏視窗,**離開工作列與 Alt-Tab**,系統匣圖示成為唯一的回來路徑。視窗位置在最小化前保存,再開啟時回到原位。
- **原生最小化(Show Desktop、Win+D、工作列右鍵)也一併收進系統匣**：面板是無邊框視窗,但這些系統層級的操作仍能觸發原生最小化。若只改按鈕,同一個動作會因為觸發方式不同而有兩種結果。
- 從系統匣重新開啟改用 `show()` 而非 `restore()`——`restore()` 對已隱藏的視窗不會有反應,會讓系統匣圖示看起來像壞掉。

## [0.35.0] - 2026-08-02

### 新增
- **Windows 面板補上視窗與產品入口**：所有面板新增原生最小化按鈕；HTML 功能表與系統匣選單新增「關於」，顯示目前程式版本與官方 GitHub 網址。

### 變更
- **面板入口名稱與動作更明確**：「更換面板」改名為「功能表」，並把 AI 圓桌討論加入該功能表；原本容易和程式更新混淆的「立即更新」統一改為「更新用量」。
- **路線圖改用 Phase A–D 表示未來工作順序**：不再把尚未完成的里程碑綁定已被實際發版使用的 v0.32–v0.35 版號，避免把規劃誤讀成 release 內容。

### 修正
- **最小化後可由系統匣正常還原**：面板現在會追蹤最小化狀態；再次點擊系統匣圖示時會還原視窗，而不是因仍被視為可見而把它隱藏。

## [0.34.0] - 2026-07-31

### 新增
- **面板與 `--doctor` 現在共用同一套健康判定**：新增 `provider_probe`，是唯一把檔案變成事實的地方，兩個介面都呼叫它。先前兩邊用各自的程式碼讀同樣的檔案，這正是它們對同一個情況給出不同說法的原因——而共用模型本來就是為了解決這件事。面板 payload 每個 provider 都帶 `state`／`reason`／`nextStep`／`updatedAt`，文字由同一組 i18n key 解析。健康狀態取得失敗時面板照常渲染：**診斷不該成為畫面空白的原因**。

### 測試
- **符號連結分支在無權限的機器上也覆蓋得到（P4）**：`Path.symlink_to()` 需要 `SeCreateSymbolicLinkPrivilege`，實查確認該權限**根本不在 token 裡**（不是「有但停用」），所以只剩開啟開發人員模式或提升權限兩條路——都是系統設定，由維護者決定。但被測分支的契約是「`lstat().st_mode` 不是一般檔案就跳過」，所以新增一條測試讓 `lstat` 對一個真實、過期、名稱吻合的檔案回報 `S_IFLNK`，走完同一條分支，並配一個控制組證明同一檔案沒偽裝就會被刪。本機現在覆蓋得到目錄、junction、符號連結三種輸入。
...
### 新增
- **人才市場改為安裝進每一個實際存在的 AI 工具**：先前只寫進 `~/.claude/agents/`，所以 Codex 使用者裝了角色卻叫不出來。現在會偵測這台機器上實際有哪些工具並各自安裝——Claude Code 用 `~/.claude/agents/<id>.md`（YAML frontmatter），Codex 用 `~/.codex/agents/<id>.toml`（`developer_instructions` 區塊）。**兩種格式不能互相複製**，所以分別產生；沒安裝的工具完全不碰。偵測依據是該工具自己的設定目錄存不存在。
...
- **新增 4 組人才市場、共 12 個角色**：文字工作台（技術文件、無情編輯、在地化）、產品決策組（範圍削減、使用者訪談、定位批評）、資料分析組（SQL 審查、指標定義、圖表批評）、維運與安全（事故處理、威脅建模、相依稽核）。連同既有的工程小隊共 5 組 15 個角色，全部雙語。
- **面板顯示角色裝到了哪些工具**：安裝成功時刻意不彈對話框，所以改由面板狀態列出工具名稱——否則使用者無從得知 Codex 到底有沒有拿到副本。
- **檔案大小閘門**（移植自上游 `8d26748`）：`wintray.py` 設 1900 行上限。上游把同類巨石拆過兩次都長回去，因為「保持精簡」只寫在文件裡、沒有東西在擋。**降低上限是對的，調高上限是政策死掉的方式。**
- **README 新增「專案架構」章節**：根目錄 56 個 `.py` 分組說明，並標出五個**不能移動**的 hook 腳本。

### 變更
- **擁有權與飄移偵測改為逐工具記錄**：某個工具出現同名但非本程式寫入的檔案時，安裝先備份再覆寫並回報是哪個工具；移除時只要**任何一個**工具的副本不是我們的就整個拒絕——只刪我們擁有的、留下其他的，使用者會拿到半殘的角色而無從解釋。舊版狀態檔仍可讀。
- **上游追蹤的分流判準更正**：先前把「所有檔案在本 fork 都不存在」當成可自動略過，那會連 macOS 專屬修正背後**可移植的概念**一起丟掉——而移植概念正是這個 fork 的目的。現在只有純資料同步才自動略過；只動到本 fork 沒有的程式碼者改列「需要判斷是否值得移植」，附路徑、要人看。

### 修正
- **文件現況漂移**：移除已發布至 v0.31.2、卻仍重複留在 `Unreleased` 的舊內容；README 比較表也改為如實標示本 fork 已提供 AI 人才市場。
- **測試會寫進使用者真實的 `~/.codex/agents/`**：多工具安裝上線後，測試 fixture 只 patch 了 Claude 的路徑。
- **`persona_store` 的目標清單在 import 時就凍住路徑**，使得測試 patch 不到、會寫進真實家目錄。改為呼叫時才建構。
### 變更
...
### 變更
- **報紙面板更有「泛黃古紙」的味道**：底色改成更深的琥珀色、加了一個很淡的羅盤浮水印、標題文字改用墨色陰影，外框也換成雙線裝飾框。
