...
def test_manual_release_rebuild_requires_the_tag_commit_sha() -> None:
    """A rebuild must pin the existing tag before replacing its assets."""
    workflow = (ROOT / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")

    assert re.search(r"(?m)^      tag_sha:\n(?:        .*\n)*?        required: true$", workflow)
    assert 'echo "ref=refs/tags/$INPUT_TAG"' in workflow
    guard_header = (
        "- name: Verify manual rebuild tag SHA\n"
        "        if: github.event_name == 'workflow_dispatch'"
    )
    assert guard_header in workflow
    assert "EXPECTED_TAG_SHA: ${{ inputs.tag_sha }}" in workflow
    assert "^[0-9a-fA-F]{40}$" in workflow
    assert 'actual_tag_sha="$(git rev-parse HEAD)"' in workflow
    assert '"${actual_tag_sha,,}" != "${EXPECTED_TAG_SHA,,}"' in workflow
    assert workflow.index("Verify manual rebuild tag SHA") < workflow.index("astral-sh/setup-uv")
