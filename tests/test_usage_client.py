...
def _write_desktop_samples(path: Path, samples: list[dict[str, object]]) -> None:
    path.write_text(
...
def test_read_desktop_usage_snapshot_reads_latest_local_plan_sample(
    ...
    assert snapshot.current_reset_estimated is False
    assert snapshot.weekly_reset_estimated is False
...
def test_read_desktop_usage_snapshot_estimates_weekly_reset_from_best_transition(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    now = 1_800_000_000.0
    weekly_boundary = now - (2 * 86400)
    path = tmp_path / "plan-usage-history.json"
    monkeypatch.setattr(usage_client, "CLAUDE_DESKTOP_USAGE_FILE", str(path))
    monkeypatch.setattr("usage_client.time.time", lambda: now)
    _write_desktop_samples(
        path,
        [
            {
                "t": (weekly_boundary - (7 * 86400) - 3600) * 1000,
                "org": "test-org",
                "u": {"fh": 0, "sd": 74},
            },
            {
                "t": (weekly_boundary - (7 * 86400)) * 1000,
                "org": "test-org",
                "u": {"fh": 0, "sd": 4},
            },
            {
                "t": (weekly_boundary - 300) * 1000,
                "org": "test-org",
                "u": {"fh": 0, "sd": 81},
            },
            {
                "t": weekly_boundary * 1000,
                "org": "test-org",
                "u": {"fh": 0, "sd": 3},
            },
            {
    ...
    assert snapshot is not None
    assert snapshot.weekly_reset_at == weekly_boundary + (7 * 86400)
    assert snapshot.weekly_reset_estimated is True


def test_read_desktop_usage_snapshot_estimates_active_five_hour_reset(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    now = 1_800_000_000.0
    session_boundary = now - 1800
    path = tmp_path / "plan-usage-history.json"
    monkeypatch.setattr(usage_client, "CLAUDE_DESKTOP_USAGE_FILE", str(path))
    monkeypatch.setattr("usage_client.time.time", lambda: now)
    _write_desktop_samples(
        ...
        [
            {
                "t": (session_boundary - 300) * 1000,
                "org": "test-org",
                "u": {"fh": 0, "sd": 20},
            },
            {
                "t": session_boundary * 1000,
                "org": "test-org",
                "u": {"fh": 3, "sd": 20},
            },
            {
    ...
    assert snapshot is not None
    assert snapshot.current_reset_at == session_boundary + (5 * 3600)
    assert snapshot.current_reset_estimated is True


def test_read_desktop_usage_snapshot_does_not_invent_inactive_session_reset(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    now = 1_800_000_000.0
    path = tmp_path / "plan-usage-history.json"
    ...
    assert snapshot is not None
    assert snapshot.current_percent == 0
    assert snapshot.current_reset_at is None
    assert snapshot.current_reset_estimated is False


def test_time_adjusted_expires_session_estimate_and_rolls_weekly_phase(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    now = 2_000_000.0
    monkeypatch.setattr("usage_client.time.time", lambda: now)
    snapshot = usage_client.UsageSnapshot(
        current_percent=30,
        current_reset_at=now - 10,
        weekly_percent=40,
        weekly_reset_at=now - 20,
        current_status="",
        polled_at=now - 30,
        data_source="claude-desktop",
        current_reset_estimated=True,
        weekly_reset_estimated=True,
    )

    adjusted = usage_client._time_adjusted(snapshot)

    assert adjusted.current_percent == 0
    assert adjusted.current_reset_at is None
    assert adjusted.current_reset_estimated is False
    assert adjusted.weekly_percent == 0
    assert adjusted.weekly_reset_at == now - 20 + usage_client.WEEKLY_SECONDS
    assert adjusted.weekly_reset_estimated is True


def test_fresh_desktop_parse_matches_cached_expiry_adjustment(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "samples": [
                    {"t": (boundary - 60) * 1000, "org": "org-a", "u": {"fh": 0, "sd": 40}},
                    {"t": boundary * 1000, "org": "org-a", "u": {"fh": 12, "sd": 5}},
                    {"t": (boundary + 60) * 1000, "org": "org-a", "u": {"fh": 14, "sd": 6}},
                ]
            }
        ),
        encoding="utf-8",
    ...
    monkeypatch.setattr(usage_client, "CLAUDE_DESKTOP_USAGE_FILE", str(path))
    monkeypatch.setattr("usage_client.time.time", lambda: boundary + 60)
    before_expiry = usage_client._read_desktop_usage_snapshot()
    assert before_expiry is not None

    after_expiry = boundary + usage_client.WEEKLY_SECONDS + 60
    monkeypatch.setattr("usage_client.time.time", lambda: after_expiry)
    cached = usage_client._time_adjusted(before_expiry)
    fresh = usage_client._read_desktop_usage_snapshot()

    assert fresh == cached
    assert fresh.current_percent == 0
    assert fresh.current_reset_at is None
    assert fresh.weekly_percent == 0
    assert fresh.weekly_reset_at == boundary + (2 * usage_client.WEEKLY_SECONDS)
