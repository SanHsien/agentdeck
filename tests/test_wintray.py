...
def test_tooltip_is_truncated_to_the_windows_limit() -> None:
    """pystray raises ValueError past szTip's 128 WCHARs, and the raise is inside the
    refresh path -- so an over-long tooltip stops the panel updating (upstream 8eacc3b)."""
    state = _state()
    ...
    for row in (
        ...
        row.title = "每週用量視窗（含快取讀取）"

    text = wintray.build_tooltip(state)

    assert len(text) == wintray.TOOLTIP_MAX_LENGTH
    assert text.endswith("…")


def test_a_short_tooltip_is_left_alone() -> None:
    text = wintray.build_tooltip(_state())
...
def test_refresh_requested_while_busy_runs_once_after_current_refresh(
    ...
    monkeypatch.setattr(autoresume_scheduler, "tick", lambda *_args: None)
    ...
    while controller._refresh_runner.in_flight and time.monotonic() < deadline:
        time.sleep(0.01)

    assert calls == 2
    assert controller._refresh_runner.in_flight is False
