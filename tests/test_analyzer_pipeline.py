...
def test_read_year_ledger_merges_git_project_names() -> None:
    ledger_path = reporter.YEAR_LEDGER_PATH
    ...
    # Only project_tokens comes from a bare-repo worktree name; model_tokens and
    # agent_tokens keep whatever string the caller stored, .git and all.
...
def test_read_year_ledger_preserves_dot_git_project_name() -> None:
    ledger_path = reporter.YEAR_LEDGER_PATH
    ledger_path.write_text(
        json.dumps(
            {
...
def test_read_year_ledger_salvages_days_on_schema_mismatch() -> None:
    """A schema bump used to discard every tracked day outright -- a maintainer
    adding a field to the ledger format cost every user their entire
    contribution/wrapped history on the next launch. Days that still parse
    under the current shape must survive a schema mismatch."""
    ledger_path = reporter.YEAR_LEDGER_PATH
    ledger_path.write_text(
        json.dumps(
            {
                "schema_version": reporter._YEAR_LEDGER_SCHEMA + 1,
                "days": {
                    "2026-06-17": {
                        "total_tokens": 100,
                        "cost": 1.0,
    ...
    assert ledger["schema_version"] == reporter._YEAR_LEDGER_SCHEMA
    assert ledger["days"]["2026-06-17"]["total_tokens"] == 100


def test_read_year_ledger_empties_when_nothing_can_be_salvaged() -> None:
    ledger_path = reporter.YEAR_LEDGER_PATH
    ledger_path.write_text(
        json.dumps(
            {
                "schema_version": reporter._YEAR_LEDGER_SCHEMA,
                "days": {"2026-06-17": {"total_tokens": "not-a-number"}},
            }
        ),
        encoding="utf-8",
    )
