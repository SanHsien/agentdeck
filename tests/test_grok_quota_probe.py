...
from providers import grok_quota_probe
...
def test_load_quota_reads_reset_week_snapshot_without_percent(grok_log: Path) -> None:
    # xAI's own log line right after a weekly reset: creditUsagePercent is left out of
    # the snapshot entirely when the used percentage is zero, not sent as 0.
    payload = json.dumps(
        {
            "ts": "2026-09-02T09:19:32.423Z",
            "msg": "billing: fetched credits config",
            "ctx": {
                "config": {
                    "currentPeriod": {"end": "2099-09-08T15:50:08.729634+00:00"},
                    "historyLen": 0,
                },
                "subscriptionTier": "SuperGrok Lite",
            },
    ...
    grok_log.write_text(payload + "\n", encoding="utf-8")

    result = grok_quota_probe.load_quota(_NOW)

    assert result is not None
    assert result.used_percent == 0.0
    assert result.period_end == "2099-09-08T15:50:08.729634+00:00"
