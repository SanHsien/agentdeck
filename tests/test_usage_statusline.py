...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1_000, "1k"),
        (999_499, "999k"),
        # 999,500 rounds to "1000k" under the k branch -- a unit that does not exist.
        (999_500, "1.0M"),
        (999_999, "1.0M"),
        (1_000_000, "1.0M"),
        (999_949_999, "999.9M"),
...
def test_fmt_tokens_never_prints_a_unit_that_does_not_exist(value: int, expected: str) -> None:
    """Ported from upstream 45b27ee, which hit this at the M/B boundary."""
...
def _isolate_context_burn_and_preferences_files(
    ...
    """把 context-burn 與 preferences 兩個檔案隔離到每個測試自己的 tmp_path。

    只隔離 context-burn 不夠：沒有明講 PREFERENCES_FILE 的測試會讀到開發機上真正的
    `~/.claude/agentdeck-preferences.json`，於是測試結果取決於誰的機器在跑。
    """
    monkeypatch.setattr(
        usage_statusline,
        "CONTEXT_BURN_FILE",
    ...
    monkeypatch.setattr(
        usage_statusline,
        "PREFERENCES_FILE",
        str(tmp_path / "agentdeck-preferences.json"),
    )
