...
import usage_terse_mode
import usage_terse_reminder
...
@pytest.mark.parametrize(
    ("constant", "module"),
    [
        (setup_hook.HOOK_VERSION, usage_statusline),
        (session_hooks.RESUME_HOOK_VERSION, usage_session_resume),
        (session_hooks.TERSE_HOOK_VERSION, usage_terse_mode),
        (session_hooks.TERSE_REMINDER_HOOK_VERSION, usage_terse_reminder),
    ],
    ids=["statusline", "resume", "terse", "terse_reminder"],
)
def test_each_hook_constant_matches_the_file_it_installs(constant: str, module: object) -> None:
    """Self-heal compares the constant with the `__version__` of the installed copy.
    Bump only the file and the fix never reaches `~/.claude`; bump only the constant
    and the hook is reported stale on every run. Upstream 144ecac is the same lesson:
    a fix that does not move the version stays invisible to anyone already installed.
    """
    assert constant == module.__version__  # type: ignore[attr-defined]
...
def test_unsetup_restores_agy_status_line(setup_paths: SetupHookPaths) -> None:
...
def test_statusline_switch_keeps_pre_install_backup_through_unsetup(
    ...
    assert "agentdeck" not in data
    assert "usage" not in data


@pytest.mark.parametrize("enable_again", [True, False])
def test_statusline_switch_reads_legacy_parked_status_line(
    monkeypatch: pytest.MonkeyPatch,
    setup_paths: SetupHookPaths,
    enable_again: bool,
) -> None:
    ...
    assert setup_hook.setup() == 0
    data = json.loads(setup_paths.settings.read_text(encoding="utf-8"))
    data["usage"] = {"previousStatusLine": data.pop("statusLine")}
    setup_paths.settings.write_text(json.dumps(data), encoding="utf-8")

    if enable_again:
        assert statusline_settings._enable_statusline_settings() == 0
        assert "statusLine" in json.loads(setup_paths.settings.read_text(encoding="utf-8"))
    assert setup_hook.unsetup() == 0
...
def test_unsetup_without_install_is_safe_and_is_usage_hook_detects_commands(
    ...
    assert not setup_hook._is_usage_hook(
        {"command": "python3 /tmp/agentdeck-statusline-pro.py"}
    )
    assert not setup_hook._is_usage_hook(
        {"command": "python3 /tmp/agentdeck-statusline.py.backup"}
    )


def test_lookalike_third_party_statuslines_remain_external() -> None:
    direct = {"statusLine": {"command": "python agentdeck-statusline-pro.py"}}
    forwarder = {
        "statusLine": {"command": "python agentdeck-statusline-forwarder-pro.py"}
    }

    assert setup_hook._detect_current_state(direct) == "external"
    assert setup_hook._detect_current_state(forwarder) == "external"
    assert not setup_hook._is_agy_usage_hook(
        {"command": "python agentdeck-statusline-agy.py.backup"}
    )
...
def test_migrate_legacy_usage_accepts_utf8_bom_settings(setup_paths: SetupHookPaths) -> None:
    settings = setup_paths.settings
    legacy_hook = setup_hook.LEGACY_HOOK_TARGET
    legacy_hook.write_text("legacy hook\n", encoding="utf-8")
    payload = {
