...
from providers.agy_quota_probe import AgyQuotaGroup, AgyQuotaResult, AgyQuotaWindow
...
def test_load_refresh_result_hides_card_when_agy_is_unavailable(
    ...
    monkeypatch.setattr("state.menubar_agy.find_agy", lambda: None)
    monkeypatch.setattr(
        menubar_agy,
        "load_quota",
...
def test_load_refresh_result_projects_mocked_quota(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("state.menubar_agy.find_agy", lambda: "/usr/local/bin/agy")
