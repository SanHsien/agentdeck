...
def _minimal_bundle() -> dict[str, dict[str, str]]:
    return {
        "en": {
            ...
            "reset_inactive": "Starts after next Claude use",
...
def test_usage_block_marks_estimated_and_inactive_resets(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(tui, "_load_i18n_bundle", _minimal_bundle)
    console = Console(record=True, width=60)
    ...
    console.print(
        tui._usage_block(
            ...
            1_700_000_000.0,
            "en",
            inactive_when_zero=True,
        )
    )

    rendered = console.export_text()
    assert "≈ 1d 0h" in rendered
    assert "Starts after next Claude use" in rendered
