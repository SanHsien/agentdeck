...
def test_project_donut_uses_report_total_and_groups_unlisted_usage() -> None:
    rendered = html_report._donut_svg([("shown", 60)], "en", total=100)
...
def test_contribution_month_labels_retry_a_month_blocked_by_spacing() -> None:
    starts = [date(2026, 8, 23) + timedelta(days=7 * index) for index in range(4)]
    weeks = [
        [
            {"date": (start + timedelta(days=day)).isoformat(), "tokens": 1, "level": 1}
            for day in range(7)
        ]
        for start in starts
    ]

    assert html_report._contribution_month_labels(weeks, "en") == [
        "Aug",
        "",
        "",
        "Sep",
...
def test_in_progress_week_is_not_compared_with_complete_week() -> None:
    daily: list[DailyTrendPoint] = [
        {"date": "2026-08-17", "tokens": 100, "cost": 0.0},
        {"date": "2026-08-24", "tokens": 200, "cost": 0.0},
        {"date": "2026-08-31", "tokens": 1, "cost": 0.0},
    ]

    rendered = html_report._trend_ascii(daily, "en", date(2026, 8, 31))

    assert "in progress" in rendered
    assert "This week is 2.0× of last week." in rendered
