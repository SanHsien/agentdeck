...
Everything user-facing and on-disk uses the `usage` prefix: hook filename, status filename, settings backup key, `~/.usage/` cache directory, `USAGE_*` env vars. The `usag-*` form is **legacy v0.1.x only** — kept as a read-fallback for migration, never written. Don't reintroduce it.
