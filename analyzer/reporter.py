...
def _read_year_ledger() -> _YearLedger:
    ...
    if ledger.get("schema_version") != _YEAR_LEDGER_SCHEMA:
        # A schema bump used to wipe every tracked day outright -- a maintainer
        # adding a field to the ledger format cost every user their entire
        # contribution/wrapped history on next launch. Individual days are
        # still parsed through _year_day_from_json below, which already
        # rejects malformed shapes, so salvage what still parses instead.
        logger.warning(
    ...
    if raw_days and not days:
        # Every day failed to parse -- an unreadable ledger, not an empty one.
        return _empty_year_ledger()
    return {
        "schema_version": _YEAR_LEDGER_SCHEMA,
        "days": days,
