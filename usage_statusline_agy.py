...
# Two languages, like every other stdlib-only hook here: usage_lang cannot be
# imported from a script Antigravity runs with its own interpreter, so the
# shipped set is duplicated. Change one, change all of them.
...
def _statusline_detect_lang(env: Optional[Dict[str, str]] = None) -> str:
    ...
    # Every Chinese locale resolves to zh-TW, everything else to English --
     # the same two-language contract usage_lang._normalize_lang() applies. A
     # code mapping to a table this script no longer carries would silently fall
     # back to English for a reader who asked for Chinese.
    if code.split("-")[0].lower() == "zh":
        return "zh-TW"
    return "en"
...
def fmt_tokens(n: Any) -> str:
    try:
        ...
        value = 0
    # Same rounding boundaries as usage_statusline.fmt_tokens; see the note there.
...
def _split_effort_suffix(name: str) -> Tuple[str, Optional[str]]:
    ...
    effort = _EFFORT_SUFFIXES.get(tail.lower()) if separator else None
...
def _render_core(data: Dict[str, Any]) -> str:
    ...
    return " | ".join(selected) if selected else "agentdeck"
