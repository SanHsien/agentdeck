    ...
    *,
    ...
    if outcome.state == PollState.SUCCESS and outcome.snapshot is not None:
        ...
        claude_session = _quota_row(
            ...
            reset_estimated=snapshot.current_reset_estimated,
            inactive_when_zero=snapshot.data_source == "claude-desktop",
        )
        ...
        if snapshot.data_source == "claude-desktop" and not outcome.message:
            status_value = _t(
                language,
                "claude_desktop_updated",
                age=format_human_time(max(0.0, now - snapshot.polled_at), language),
            )
        else:
            status_value = _status_message_value(outcome, "status_synced", language)
        if snapshot.data_source != "claude-desktop" and (
            snapshot.is_stale or snapshot.data_source != "hook"
        ):
...
def _quota_row(
    ...
    if resets_at is None:
        reset_text = (
            _t(language, "reset_inactive")
            if inactive_when_zero and pct == 0
            else _t(language, "reset_placeholder")
        )
        warning = False
    else:
