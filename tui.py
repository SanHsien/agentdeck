    ...
    *,
    ...
    if reset_at is None:
        countdown_text = (
            ...
            if inactive_when_zero and percent == 0
            else _t(language, "resets_in_placeholder")
        )
    else:
        countdown_text = format_countdown(reset_at, language, now)
        if reset_estimated:
            countdown_text = f"≈ {countdown_text}"
    ...
    if state.snapshot is None:
        ...
        current_block = (
            _usage_block(
                ...
                now,
                state.language,
                reset_estimated=state.snapshot.current_reset_estimated,
                inactive_when_zero=state.snapshot.data_source == "claude-desktop",
            )
        ...
        weekly_block = (
            _usage_block(
                ...
                now,
                state.language,
                reset_estimated=state.snapshot.weekly_reset_estimated,
            )
