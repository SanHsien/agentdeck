...
**Set it for the current shell with `$env:`, not with `setx`.**
`UV_PROJECT_ENVIRONMENT` is a per-project variable; `setx` writes it to the Windows
User scope, which points *every* uv project on the machine at one shared environment.
Package sets then overwrite each other and tests go red for reasons unrelated to the
code (measured 2026-09-06: another repo lost `hatchling` this way and reported 23
failures that looked like a regression). `tools/dev_check.ps1` supplies this default
when the variable is unset, so you only need to set it for a manual `uv sync`.
