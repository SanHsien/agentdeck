...
## D-04：本機測試的兩個環境失敗用 `tools/dev_check.ps1` 處理，不改測試

**日期**：2026-07-29

**決定**：Windows 上兩個非 code bug 的測試失敗，在 `tools/dev_check.ps1` 這一層處理，不去動 `tests/` 本身。

**考慮過的替代方案**：在測試加 `pytest.mark.skipif` 直接跳過。

**為什麼**：這兩條在 CI 上是會過、也**應該**過的（CI runner 有符號連結權限、也沒有 Claude Code SDK 注入的環境變數）。在測試裡永久 skip 會讓真實的迴歸能力縮水；放在本機執行腳本這層，CI 的覆蓋率完全不受影響。

**做法**：
- 符號連結權限：先實測本機能不能建連結，不能才 `--deselect` 該條並印出說明。
- 環境變數塗銷：移除值長度 ≤ 4 的機密名稱環境變數（`-SkipEnvScrub` 可還原原始行為）。

**注意**：其中第二項暴露的是**真實缺陷**（`discussion_cli` 的塗銷沒有值長度下限），已記在 [`REPO_REVIEW.md`](../REPO_REVIEW.md) 的 P3，dev_check 的處置只是繞過症狀，不是修好它。
