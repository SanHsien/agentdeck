...
## D-24：用量預估被單次跳升拉爆；exe 補上版本資訊

**日期**：2026-08-13

**預估被一則大訊息毀掉（上游 `4eb0e5e`）**：`burn_rate` 原本逐間隔算速率再用 `EMA(alpha=0.5)` 合併，最新那一段永遠佔一半權重，**而且不隨間隔長短加權**。Claude 的百分比是階梯式跳動，一則大訊息落在短輪詢間隔就會產生極大的瞬時速率。

**本機重現**：穩定 0.5%/分鐘燒十分鐘，然後一則大訊息在 5 秒的間隔內加 7%——
...
面板會對使用者大喊「不到一分鐘就用完」。改用窗口端點的平均斜率，數學上等同各間隔速率按長度加權。

**修完要證明它沒有變成「永遠不警告」**：三種情境實測——尖峰 27.9 分、真的在加速 10.0 分、穩定慢燒 190 分。**只會壓低不會誤放的修正，跟拿掉功能沒兩樣**，所以測試裡同時釘住「加速時預估必須明顯短於穩定燃燒」。

**一條舊測試因此紅了，而它紅得有道理**：`test_forecast_ema_responds_to_accelerating_burn` 斷言預估必須比兩點斜率短 25%——那正是新設計刻意放棄的靈敏度，因為那份靈敏度和它的誤報是同一件事。改寫成表達新契約，而不是調數字讓它變綠。
...
結果併進報告與 `has_updates`；新增 `tests/test_upstream_tickets.py`。sync-points 的
`tickets.reviewed_pr_through` / `reviewed_issue_through` 不動。

**理由**：那兩個數字早就寫在 `docs/UPSTREAM.md` 的 sync-points 裡，但**沒有任何程式讀它們**——
檢查器只比對分支的 `last_reviewed`。PR 與 issue 不是「查過沒發現」，是根本沒查，而每週的排程
報告長得跟查過一樣綠。艦隊層級問題，見 `SanHsien/repo-fleet-ops` 的 `docs/INCIDENTS.md` 第十條
（24 個 fork 裡 21 個都這樣）。

三個實作決定：

- **走本檔既有的 `_request`（urllib + `GITHUB_TOKEN`），不引 `gh`。** commit 軸一直走 REST；
  多一條行為不同的路是負債不是方便。workflow 那一步本來就給了 `GITHUB_TOKEN`，不需要改。
- **`state=all`**：一個項目在兩次排程之間被開了又關，對本 fork 來說仍然是從沒審過；未合併就
  關閉的 PR 永遠不會出現在 commit 軸上。
- **ticket 不做 relevance 分流。** commit 軸可以自動略過上游每天的 AI digest，因為那種雜訊認得
  出來；但已審視編號以上的每一筆 ticket 都還要人讀。

**一個實作陷阱（測試抓到的）**：GitHub 的 issue 端點會把 PR 一起回傳（共用編號空間）。過濾要
用「`pull_request` 欄位在不在」判斷，**不能用真假值**——回傳空物件時 PR 會在兩個面向都被算一次。

**證據**：`ruff` / `mypy`（201 檔）全過；`pytest` 1543 passed（唯一失敗是 dev_check 已記載的
本機符號連結權限例外，CI 上不存在）；實跑檢查器 `has_updates=False`，報告多出兩節，分別停在
`#116` 與 `#92`、之後皆無新項目。

---

## D-29：issue #18 的 21 筆——採穩健性修正，報告視覺改版列後續

**日期**：2026-09-04

**背景**：上游更新檢查 issue #18 從 11 筆長到 20 筆（外加 `1312289`），三天內連開四次留言。

**決定**：`7cd04fc..1312289` 逐筆讀 diff。採用 resume／terse／hooks 的穩健性修正、Grok 週配額
歸零、pricing 版本誤配與舊模型價格、額度門檻通知、偏好檔測試隔離、Windows 登錄檔容錯、terse
禁止原文轉貼、locale `@` 修飾詞。上游 release chore、官網五語文案、HTML 報告視覺改版、macOS
`launchctl` 一律不採用或列後續，逐筆理由進 `UPSTREAM.md` 的 Skipped 表。

**取捨理由**：這一批的價值集中在「安靜壞掉」那一類——會刪掉使用者自備 hook 的子字串誤判、
非 UTF-8 終端機直接當掉、週配額歸零時卡片消失、開機時額度已高卻不提醒。這些沒有人會回報，
只會被當成「有時候怪怪的」。相對地，報告與官網的視覺改版是產品方向題，牽動已與上游分岔的
`ui/`，而且需要獨立的渲染與列印驗收，混進上游同步只會讓這一批難以審查。

**限制**：`1312289` 之後上游又推進到 `97d62e6`（十餘筆，含 PR #126 的 TUI Antigravity loader），
本輪未審視。水位誠實停在 `1312289`，下一輪排程會重新列出，issue #18 因此仍會有內容——
不是漏做。

**驗證**：`pwsh -NoProfile -File tools/dev_check.ps1` → 1560 passed、8 skipped、`PASS: 全部通過`、exit 0。

---

## D-30：`e32c2f6` 的出處補記，以及為什麼 `last_reviewed` 不跟著推

**日期**：2026-09-06

**背景**：commit `e32c2f6` 動了 17 個檔、直接推上 `main`，訊息是「fix: reconcile usage data
and model updates」，沒有寫出它從上游哪幾筆來，`docs/UPSTREAM.md` 的水位與 Skipped 表也沒動。

**決定**：逐 hunk 把它對回上游（`69e2e12`、`97d62e6` 的歷史保全半部、`2e99c86`、
`c1d2749`＋`30aac9c`、`1674ab1` 的 Fable 5.1 半部、`9282a5b`），寫進 `UPSTREAM.md` 的
2026-09-06 段落。`last_merged` 推到 `9282a5b`；**`last_reviewed` 維持 `1312289` 不動**。

**取捨理由**：issue #19 列了 50 筆，其中只有上述 6 筆現在有可查證的判定。把 `last_reviewed`
推到 tip 只需要改一個字串，代價卻是其餘四十幾筆從此不再被任何機制提起——`UPSTREAM.md`
開頭那句「`last_reviewed` 只負責『這次不用再提醒』，Skipped 表才負責『不失憶』」講的就是
這個。水位停在有證據的地方，issue #19 因此仍會有內容，那是正確狀態不是漏做。

**限制／可疑處**：

- 對應關係是從 diff 內容重建的，不是原提交自己聲明的。`usage_notifications.py` 把門檻判斷
  改寫成 list comprehension 那一 hunk 對不回上游任何一筆，行為與 2026-09-04 採用的
  `b57e8e9` 相同，判為本地重構，已在文件裡列為已知的未解釋改動。
- 過程中一度誤判「那個 commit 帶著壞掉的狀態推上去」——初次跑閘門 mypy 與 pytest 都紅。
  實查後是本機 venv 少了 windows extra（`No module named 'PIL'`／`pystray`），`mypy .`
  單獨跑是 `Success: no issues found in 204 source files`。**跑閘門前先確認環境是
  `uv sync --frozen --group dev --extra windows` 的狀態**，否則會把環境問題讀成程式碼問題。

**驗證**：`uv sync --frozen --group dev --extra windows` 後 `pwsh -NoProfile -File
tools/dev_check.ps1` → 1571 passed、8 skipped、`PASS: 全部通過`、exit 0。

---

## D-31：issue #20 那一輪——採三筆缺陷修復，Windows 高 DPI 與 `ui/` 視覺列後續

**日期**：2026-09-12

**決定**：`0bd03fb..2cdf3a0` 共 19 筆，採用 `45b27ee`（狀態列單位邊界）、`93b2f19`（精簡模式讓步於
使用者明確要求）、`8eacc3b`（系統匣提示截斷）三筆，並把 `144ecac` 的觀念套在本次真的改到的檔案上；
`42d6ad1`／`c4f0edf`（`ui/` 視覺）、`2d606b1`（高 DPI）、`a2cd93e`（設定目錄集中化）列後續；其餘不適用。
`last_reviewed` 推到 `2cdf3a0`（上游 tip）、`last_merged` 推到 `8eacc3b`（三筆採用中最新的那一筆）。

**考慮過的替代方案**：把 `2d606b1` 一起移植。上游那筆同時修「面板位置」「跨螢幕記憶」「底部裁切」，
對 Windows 使用者是有感的缺陷。

**為什麼不**：上游改的是它自己新加的 `wintray/app.py` 與 `panels/panel_scale.py`，本 fork 的系統匣是
另一套頂層 `wintray.py`，沒有那兩個檔——移植是概念重寫，而且驗收條件是**多螢幕、混合縮放的實機操作**，
不是跑測試能收斂的。更關鍵的是上游這條線還沒收斂：PR `#134`／`#135` 仍開著，都是同一個問題的後續修正。
現在跟進等於移植一個上游自己還在改的設計。等它收斂再一次做完，成本較低也較不會白工。

**後果**：本 fork 在高 DPI 螢幕上是否重現上游 issue `#130`（250% 縮放下面板開到螢幕邊緣）**未知**，
要實機驗。這是列後續時的第一個檢查點，不是已排除的風險。

**附帶發現（比原任務更重要）**：本 fork 用「常數 vs 已安裝檔案的 `__version__`」判斷 hook 要不要換新，
共四組配對（statusline／resume／terse／terse reminder），但**沒有任何測試防止兩者分家**。只改檔案不升
版號＝修正永遠不會進到 `~/.claude` 的副本；只升常數不改檔案＝每次自我修復都判定過期。已補一條參數化
契約測試把四組全釘住。上游 `144ecac` 修的正是前一種情況。

**連帶的切分**：加上長度上限後 `wintray.py` 變成 1863 行，超過 `scripts/check_file_size.py` 守的 1860 行
上限。該檔的政策明寫「把新邏輯搬進葉模組，不要調高上限」，所以把 `tray_icon_style` 與 `build_tooltip`
這兩個純函式移到新的 `tray_text.py`（`wintray.py` 保留同名再匯出，呼叫端與測試不動），`wintray.py` 降到
1816 行，上限也照政策一起降到 1816——切分後不降上限，等於只是替下一個功能買了成長空間。

**審查抓到的已知缺口（這一輪不修，理由在此）**：`usage_statusline_agy.py` **沒有版號、也沒有自我修復
比對**。`setup_hook._setup_agy()` 只在使用者把 Antigravity 狀態列從關切到開時用 `shutil.copyfile` 複製一次，
所以這次的 `fmt_tokens` 修正只會進到新安裝的副本；已經裝好的人會一直看到 `1000k`。新加的四組配對契約測試
結構上也蓋不到這第五個檔。

不在這一輪順手補，是因為那要動安裝與自我修復的路徑（新增版號常數、在 self-heal 加一支比對、決定舊副本
沒有版號時怎麼判定），屬於會改寫使用者環境的程式，驗收要含實機的開→關→再開流程，和「移植三筆上游修正」
是兩件事。**觸發條件**：下次動到 `usage_statusline_agy.py` 或 `_setup_agy()` 時一起做；在那之前，這個檔的
修正只對新安裝生效是已知且寫下來的狀態，不是被遺漏。

**驗證**：`uv sync --frozen --group dev --extra windows` 後
`pwsh -NoProfile -File tools/dev_check.ps1` → **1608 passed、8 skipped、1 deselected，`PASS: 全部通過`、
