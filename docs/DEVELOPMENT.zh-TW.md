...
**用 `$env:` 設定當前 shell，不要用 `setx`。** `UV_PROJECT_ENVIRONMENT` 是 per-project
變數；`setx` 會寫進 Windows User 範圍，讓機器上**每一個** uv 專案共用同一個虛擬環境。
症狀是別的 repo 的 `.venv` 消失、套件集合被互相覆寫，而測試會以與程式碼無關的理由變紅
（2026-09-06 實測：另一個 repo 因此少了 `hatchling`，假造出 23 筆紅燈，差點被當成迴歸）。
`tools/dev_check.ps1` 在變數未設定時會自己補上這個預設值，所以只有手動跑 `uv sync` 才需要先設。
