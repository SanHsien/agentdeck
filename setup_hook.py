...
# Must equal `usage_statusline.__version__`: `needs_update()` compares this constant
...
def _command_references(command: str, filename: str) -> bool:
    """Match an installed script filename without claiming lookalike tools."""
    return bool(
...
def _migrate_from_legacy_usage() -> None:
    ...
    if settings is not None:
        try:
            ...
            if (
                isinstance(cmd, str)
                and _command_references(cmd, LEGACY_HOOK_TARGET.name)
...
def _is_agy_usage_hook(status_line: object) -> bool:
    ...
    return isinstance(command, str) and _command_references(command, AGY_HOOK_TARGET.name)
