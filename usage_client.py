...
DESKTOP_RESET_MAX_GAP_SECONDS = 2 * 3600
DESKTOP_RESET_MIN_DROP = 10
...
class UsageSnapshot:
    ...
    current_reset_estimated: bool = False
    weekly_reset_estimated: bool = False
...
def _read_desktop_usage_snapshot() -> UsageSnapshot | None:
    ...
    parsed: list[tuple[float, object, int | None, int | None]] = []
    for sample in samples:
        ...
        if five_pct is None and seven_pct is None:
            continue
        parsed.append((sampled_at_ms / 1000, sample.get("org"), five_pct, seven_pct))

    if not parsed:
        return None
    parsed.sort(key=lambda item: item[0])
    polled_at, latest_org, five_pct, seven_pct = parsed[-1]
    same_org = [sample for sample in parsed if sample[1] == latest_org]
    now = time.time()

    current_reset_at: float | None = None
    current_reset_estimated = False
    if five_pct is not None and five_pct > 0:
        for previous, current in reversed(list(zip(same_org, same_org[1:], strict=False))):
            gap = current[0] - previous[0]
            previous_pct = previous[2]
            current_pct = current[2]
            if (
                previous_pct is None
                or current_pct is None
                or gap <= 0
                or gap > DESKTOP_RESET_MAX_GAP_SECONDS
            ):
                continue
            boundary = (previous_pct == 0 and current_pct > 0) or (
                previous_pct - current_pct >= DESKTOP_RESET_MIN_DROP
            )
            candidate = current[0] + FIVE_HOUR_SECONDS
            if boundary and candidate > polled_at:
                current_reset_at = candidate
                current_reset_estimated = True
                break

    weekly_reset_at: float | None = None
    weekly_reset_estimated = False
    transitions: list[tuple[float, float]] = []
    for previous, current in zip(same_org, same_org[1:], strict=False):
        gap = current[0] - previous[0]
        previous_pct = previous[3]
        current_pct = current[3]
        if (
            previous_pct is None
            or current_pct is None
            or gap <= 0
            ...
            continue
        if previous_pct - current_pct >= DESKTOP_RESET_MIN_DROP:
            transitions.append((gap, current[0]))
    if transitions:
        _, weekly_reset_at = min(transitions, key=lambda item: item[0])
        while weekly_reset_at <= polled_at:
            weekly_reset_at += WEEKLY_SECONDS
        weekly_reset_estimated = True

    return _time_adjusted(UsageSnapshot(
        current_percent=five_pct,
        current_reset_at=current_reset_at,
        weekly_percent=seven_pct,
        ...
        current_reset_estimated=current_reset_estimated,
        weekly_reset_estimated=weekly_reset_estimated,
    ))
...
def _time_adjusted(snapshot: UsageSnapshot) -> UsageSnapshot:
    ...
    five_reset = snapshot.current_reset_at
    five_estimated = snapshot.current_reset_estimated
    if (
        five_pct is not None
        and five_reset is not None
        and five_reset < now
    ):
        five_pct = 0
        if five_estimated:
            five_reset = None
            five_estimated = False
    seven_pct = snapshot.weekly_percent
    seven_reset = snapshot.weekly_reset_at
    if (
        seven_pct is not None
        and seven_reset is not None
        and seven_reset < now
    ):
        seven_pct = 0
        if snapshot.weekly_reset_estimated:
            while seven_reset <= now:
                seven_reset += WEEKLY_SECONDS
    return UsageSnapshot(
        current_percent=five_pct,
        current_reset_at=five_reset,
        weekly_percent=seven_pct,
        ...
        current_reset_estimated=five_estimated,
        weekly_reset_estimated=snapshot.weekly_reset_estimated,
    )
