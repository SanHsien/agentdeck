...
h1,.wrapped-copy h3,.wrapped-total,.prompt,.share-modal h2,.share-section h3,.persona-card h3{
...
.eyebrow,.meta,.empty,footer,.cursor,.prompt,.share-trigger,.share-close,.share-action,.tokens,.cost,.pct,.trend-row .week,.trend-row b,.trend-row em,.delta,.donut-total,.donut-sub,.sub-plan,.sub-since,th,td,.wrapped-kicker,.wrapped-total-label,.contribution-months,.contribution-days,.contribution-legend{
...
.cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;margin:28px 0 20px}
.card{
...
.card i{display:block;font-style:normal;color:var(--muted);font-size:.78rem;line-height:1.5;padding-top:10px;overflow-wrap:anywhere;letter-spacing:0}
...
.section{
  background:var(--panel);border-radius:12px;margin-top:20px;padding:24px 20px;border:1px solid var(--card-border);box-shadow:0 8px 32px 0 var(--card-shadow)
}
...
.rank-line{padding:12px;color:var(--text-soft);border:1px solid transparent;border-radius:10px;box-shadow:inset 0 -1px 0 rgba(255,255,255,.03);transition:transform .2s ease,background-color .2s ease,border-color .2s ease,box-shadow .2s ease}
...
.tool-row{padding:16px;border:1px solid var(--card-border);border-radius:12px;background:var(--soft);transition:transform .25s cubic-bezier(.4,0,.2,1),border-color .25s cubic-bezier(.4,0,.2,1),box-shadow .25s cubic-bezier(.4,0,.2,1)}
...
.wrapped-section{background:linear-gradient(135deg,rgba(143,134,201,.10),rgba(90,191,160,.13))!important;border:1px solid rgba(255,255,255,.15)!important}
@media (prefers-color-scheme: light){
  .wrapped-section{background:linear-gradient(135deg,rgba(104,96,155,.07),rgba(46,125,104,.08))!important;border:1px solid rgba(255,255,255,.5)!important}
}
.wrapped-card{display:grid;grid-template-columns:minmax(0,1.15fr) 160px minmax(260px,.95fr);gap:20px;align-items:center;padding:4px}
...
.wrapped-total{margin-top:18px;max-width:100%;font-size:clamp(1.5rem,2.2vw,2.15rem);line-height:1;font-weight:800;letter-spacing:-.05em;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:clip;font-variant-numeric:tabular-nums}
...
.wrapped-art img{width:min(160px,100%);height:auto;image-rendering:auto;filter:drop-shadow(0 14px 28px rgba(0,0,0,.42)) drop-shadow(0 0 22px rgba(90,191,160,.24))}
@media (prefers-color-scheme: light){
...
.wrapped-metric:last-child{grid-column:1/-1}
...
@media (max-width:480px){
  ...
  .trend-row{grid-template-columns:48px minmax(0,1fr) 64px 72px;gap:10px;padding:8px 10px}
