...
def _trend_ascii(daily: list[DailyTrendPoint], lang: str, date_to: date) -> str:
    ...
    summary = (
        f'<div class="trend-summary">{_escape(_trend_summary(weekly, lang, date_to))}</div>'
    )
...
def _render_trend_section(data: Mapping[str, Any], lang: str, date_to: date) -> str:
    return _section(
        ...
        _trend_ascii(data.get("daily_trend", []), lang, date_to),
    )


def _contribution_month_labels(weeks: list[list[dict[str, Any]]], lang: str) -> list[str]:
    labels: list[str] = []
...
def _render_contribution_section(data: Mapping[str, Any], lang: str) -> str:
    ...
    month_labels = _contribution_month_labels(weeks, lang)
...
{insight_surface}  {_render_tools_section(report_data, lang)}
  ...
  {_render_trend_section(report_data, lang, date_to)}
