...
async function downloadHtml(maskProjects) {
  const html = await withShareableReport(maskProjects, async () => (
    '<!doctype html>\\n' + document.documentElement.outerHTML
  ));
  const blob = new Blob([html], {type: 'text/html'});
  downloadBlob(blob, buildShareFilename('html'));
}
...
async function downloadPng(maskProjects) {
  if (!reportRoot || !window.htmlToImage?.toBlob) {
    throw new Error('html-to-image unavailable');
  }
  const blob = await withShareableReport(maskProjects, async (node) => (
    window.htmlToImage.toBlob(node, {cacheBust: false, pixelRatio: window.devicePixelRatio || 1})
  ));
  if (!blob) {
    throw new Error('PNG export returned an empty blob');
  }
  downloadBlob(blob, buildShareFilename('png'));
}
...
document.addEventListener('click', async (e) => {
  ...
  const maskProjects = Boolean(shareFileMask?.checked);
  try {
    if (action === 'download') {
      await downloadHtml(maskProjects);
      return;
    }
    if (action === 'csv') {
      await downloadCsv(maskProjects);
      return;
    }
    if (action === 'png') {
      await downloadPng(maskProjects);
    }
  } catch (error) {
    console.error(error);
  }
