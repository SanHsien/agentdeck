...
<#
.SYNOPSIS
    跑本專案的 CI gate：lock、ruff、mypy、雙語文件、檔案大小、pytest。

.DESCRIPTION
    等同 .github/workflows/check.yml。全綠才能 commit。

    這個腳本不安裝或移除依賴；只有 checkout 位於 OneDrive 且使用者未指定環境時，
    會在目前程序設定 UV_PROJECT_ENVIRONMENT=C:\tmp\agentdeck。CI 不在 OneDrive，
    因此仍使用標準 .venv。另一個本機差異是符號連結權限，CI 上不存在
    （見下方 pytest 段落）。
...
# placeholder tree. Honour an explicit value; otherwise supply the documented
# default ourselves when this checkout is inside OneDrive.
#
# Setting it here rather than machine-wide is deliberate. It is a per-project
# variable, and a `setx` into the Windows User scope points every uv project on
# the machine at one shared environment -- package sets overwrite each other and
# unrelated repositories go red (2026-09-06: a sibling repo lost `hatchling` that
# way and reported 23 failures that read as a regression). Scope it to this
# process, which also leaves CI alone: it never sets the variable and never runs
# from OneDrive, so it keeps using .venv.
if (-not $env:UV_PROJECT_ENVIRONMENT) {
    $inOneDrive = $PSScriptRoot -like '*\OneDrive\*'
    if ($inOneDrive) {
        $env:UV_PROJECT_ENVIRONMENT = 'C:\tmp\agentdeck'
    }
}
...
# Ported from upstream 8d26748: a size ceiling on the module every feature
# is tempted to grow into. Lower it when a cut lands; never raise it.
Invoke-Gate 'file-size' @('run', '--no-sync', 'python', 'scripts/check_file_size.py')
