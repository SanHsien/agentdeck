...
- **Claude Code quota no longer freezes on stale values or `--` for Claude Desktop-only use.** Desktop runs Claude Code in stream-json mode and does not refresh the terminal-only statusLine hook, so agentdeck kept replaying an old `agentdeck-status.json` and zeroed its percentages after the recorded reset. It now reads the local `plan-usage-history.json` that Claude Desktop keeps current, taking over only when its sample is newer than the hook. That file has no reset timestamp, so the UI keeps the real percentages and shows `Resets in --` instead of inventing a time; no Anthropic usage API is called.
