...
- **Status-line fixes now actually reach the installed copy.** `setup_hook` decides whether to replace the copy under `~/.claude` by comparing versions, so a fix that changes the file without bumping its version ships to nobody. The versions now move together, and a test pins the constant to the file it installs. Same lesson as upstream `144ecac`.
- **Grok spend is no longer stuck at `$0.00`.** Costs are now read from `costUsdTicks` in each Grok CLI session's `updates.jsonl`, and requests that have rolled out of `unified.jsonl`'s window are restored as their own entries. Ported from upstream `784eeb9` / `7a78e64` through a mechanical rewrite verified byte-for-byte.
- **The offline price table now covers Codex and newer Claude models.** When the online price table is unavailable, `gpt-6-astra`, `gpt-5.6-terra` / `sol` / `luna`, `gpt-5.5`, `gpt-5.4-mini`, and `claude-mythos-5-1` no longer cost zero. Ported from upstream `47b0a64`.
- **Rounded token counts no longer produce the nonexistent `1000.0K` or `1000.0M` units.** Formatting thresholds now match the actual rounding boundaries: `999,950` becomes `1.0M`, and `999,950,000` becomes `1.00B`; regression tests cover both boundaries. Ported from the formatting half of upstream `d665d67`.
- **Manual release-asset rebuilds now require the tag's full commit SHA.** `workflow_dispatch` has a required SHA input and checkout is pinned to `refs/tags/<tag>`; if the checked-out commit differs from that SHA, the job stops before dependency installation or build instead of letting a moved or mistyped tag overwrite existing assets via `--clobber`.

### Changed
- **The tray's text building moved to `tray_text.py`.** `tray_icon_style` and `build_tooltip` are pure functions; moving them out brought `wintray.py` down to 1,816 lines, and its guarded ceiling was lowered to match. No behavioural change.

### Documentation
- **The project site now reports the correct four panel themes.** English and Traditional Chinese metadata, feature cards, and gallery copy still said ten; they now match the shipped Classic, Catppuccin, Stained Glass, and Origami themes.

## [0.42.0] - 2026-09-01

### Added
- **A fourth local quota card for Grok CLI.** Reads the weekly credit snapshot (`billing: fetched credits config`) and per-request tokens (`shell.turn.inference_done`) that Grok CLI already writes to `~/.grok/logs/unified.jsonl`. Does not start grok or call an xAI usage API. That source has no session / burn-rate data, so the card shows a single weekly bar; per-request tokens fold into the panel's today cost/spend. Wired into all four themes, Hide Sections, the tray tooltip, card drag regions, and card order. Taken from upstream `505336f` / `74ad95f` / `5463cd3` / `32da5ab`, plus the Windows drag-region gap from `1cc5929`. The card stays hidden when `~/.grok` or a current-week snapshot is missing — the same "no data, no slot" contract as Antigravity.
...
### Fixed
- **The per-message terse reminder hook was never updated once installed.** `_self_heal_terse_reminder()` only checked whether the script or the UserPromptSubmit entry was *missing* and returned early when both existed, so an installed-but-stale copy stayed stale forever and upgrading agentdeck never delivered a newer reminder. The other three hooks in the same `session_hooks.py` (SessionStart terse, Codex terse, resume) already compare versions, and `usage_terse_reminder.py` already carried a `__version__` that nothing read. Adds `TERSE_REMINDER_HOOK_VERSION` and `_installed_terse_reminder_version()`; the restore path for a missing artifact is unchanged, with a new "present but stale" branch that reinstalls and logs `update_terse_reminder_hook`. Taken from upstream `1ec6fe4`.
- **The terse-mode instruction contradicted itself: the same paragraph required a 🐾 in the opening greeting and banned emoji.** A second sentence banned tool-call narration outright, which collides with setups that ask for one line of intent before each tool run. Both are rewritten: emoji are banned in the body but allowed in the opening greeting, and reciting tool names stays discouraged while a single line of intent before running a tool is explicitly fine. **Both copies had to change** — `_DEFAULT_INSTRUCTION` in `usage_terse_mode.py` is only the fallback; the text that actually reaches the model is `terse_mode_instruction` in `i18n.json`, which `_write_terse_sidecar()` writes to `~/.claude/agentdeck-terse-prompt.json` and `_load_instruction()` prefers. `TERSE_HOOK_VERSION` moves to 1.1 so users who already have terse mode on pick up the corrected text on the next self-heal. Taken from the prompt half of upstream `4222250`; the language-detection half does not apply here, because this fork's `_detect_lang()` already falls back to `_windows_system_lang()` and never defaults to English.
- **The Windows packager now asserts that the exe archive actually contains `wintray` and `tui`.** Upstream v0.29.34–36 renamed those modules into packages and left hidden-import names stale, so the shipped exe existed and died on launch with `ModuleNotFoundError` while `Test-Path` stayed green. This fork still uses top-level `wintray.py` / `tui.py`, so copying `wintray.app` / `tui.app` would break the bundle; the port is `c1b8d80`'s archive_viewer assertion with this fork's module names. `82895b6` is skipped because that bug does not exist here.
- **The suite read the operator's real usage data.** `test_report_today_uses_codex_token_count_deltas` asserts 65 tokens and read **21,786,335** on a machine that actually runs agentdeck: it redirected `SESSIONS_DIR`, but `codex_loader` also has `ARCHIVED_SESSIONS_DIR`, and `_seed_caches_from_disk()` reloads real entries from `~/.agentdeck/codex_jsonl_cache.json` before anything is parsed. CI stayed green throughout -- a fresh runner has neither `~/.agentdeck` nor `~/.codex`. `tests/conftest.py` gains an autouse fixture that points every provider and adapter source directory and disk cache (including the constants the adapters capture at import time) at a per-test temporary directory and resets the seeded flags, so missing one fails closed and empty rather than reading somebody's data. `tests/test_operator_data_isolation.py` pins each of those constants, so a new un-isolated path fails the suite.
- **A JSONL line had no size limit, so one bad line could take the process down.** Session logs are written by other programs and are not trusted input, yet every read path called `readline()` and allocated whatever the line happened to be. A new `jsonl_limits` module caps a line at 64 MiB -- upstream picked that from measured data where the largest real line was 23 MB -- and an oversized line is now drained, logged as a warning, and skipped while the rest of the file still parses. `json.loads` also raises `RecursionError`, not `JSONDecodeError`, on deeply nested input; four parse sites caught only the latter, so one nesting bomb made a whole log unreadable. Both are fixed, taken from upstream `2588cc0`.
  One thing this fork does differently: the incremental caches in `history_loader` and `codex_loader` keep a rolling digest of the confirmed prefix and re-verify it by hashing the file from offset 0 on the next run. Porting upstream's version as-is advances the confirmed offset past a drained line without hashing the bytes that were skipped, so the stored digest disagrees with the file forever -- entries stay correct, but the incremental path silently degrades into a full re-parse on every run. `read_bounded_jsonl_line` therefore takes an `on_skipped_bytes` callback, and those two call sites pass `digest.update`; `test_skipped_oversized_line_keeps_the_incremental_cache_usable` fails without it.
- **Claude Code quota and reset information now follow local history for Claude Desktop-only use.** Desktop runs Claude Code in stream-json mode and does not refresh the terminal-only statusLine hook, so agentdeck previously kept replaying an old `agentdeck-status.json`. It now reads the local `plan-usage-history.json` that Desktop keeps current, shows the source age, and estimates the five-hour and seven-day windows from same-organization usage drops. Estimates are explicitly marked with `about`, and the UI still shows `--` when there is not enough evidence. A manual refresh requested while another refresh is busy is coalesced into one trailing refresh instead of being dropped; no Anthropic usage API is called.
- **Upstream correctness and safety fixes are included.** Hook ownership now matches complete script filenames, so `agentdeck-statusline-pro.py` and backup files are not mistaken for replaceable agentdeck hooks. The HTML report counts unlisted tokens in the project donut's Other slice, keeps month labels after spacing collisions, and excludes an in-progress week from week-over-week comparisons. The Antigravity statusline also avoids repeating a reasoning-effort suffix already present in the model name.
...
### Changed
- **`ruff` floor 0.16.4 -> 0.16.5**, lock moved with it. The only REVIEW UPDATE row in the dependency freshness check; the new release adds no findings across the 200 existing files and `dev_check.ps1` stays green (1537 passed).
- **Terse mode now asks for plain language, not just short replies.** The instruction only asked for brevity, so replies got shorter by turning into jargon — terse but unreadable. Adds three checkable rules: pick the everyday word over the jargon one ("save it first", not "persist it first"), gloss an unavoidable technical term once on first use in eight words or fewer, and close with what you did, whether it worked, and what to do next. The per-message reminder hook grows by two words, so its measured per-turn saving is unaffected. Taken from upstream `ec89500`.
